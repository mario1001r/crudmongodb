...
@section('buttons')
    <form action="{{ url('/profile/user/edit') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <a href="{{ url('/profile/user') }}" class="btn btn-danger"><i class="fa-solid fa-ban"></i> @lang('generals.cancel')</a>
    ...
    @section('content')
        <div class="row">
            ...
            <div class="col-lg-3">
                <label for="phone_number_code">@lang('users.phone_code')</label>
                <select id="phone_number_code" name="phone_number_code" class="selectpicker" data-live-search="true">
                    @foreach ($countries as $country)
                        <?php
                            $flag = $country->flag != '' ? $country->flag : 'default.png';
                        ?>
                        <option value="{{$country->phone_code}}" data-subtext="<img src='{{ asset('imgs/flags/' . $flag) }}' width='25px' height='20px' style='margin-left:-92.5%;text-align:left;'>">
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;+{{$country->phone_code.' ('.$country->abbreviation.') '.$country->name}}
                        </option>
                    @endforeach
                </select>
                <input type="hidden" id="phone_code_hidden" name="phone_code_hidden" />
            </div>
            <div class="col-lg-3">
                <label for="phone_number">@lang('users.phone_number')</label>
                <input type="text" id="phone_number" name="phone_number" class="form-control"
                    value="{{ $user->partner->phone_number }}" maxlength="10"/>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-4">
                ...
                <input type="text" id="first_name" name="first_name" class="form-control"
                    value="{{ $user->partner->first_name }}" />
            </div>
            <div class="col-lg-4">
                <label for="last_name">@lang('users.last_name')</label>
                <input type="text" id="last_name" name="last_name" class="form-control"
                    value="{{ $user->partner->last_name }}" />
            </div>
            <div class="col-lg-4">
                <label for="movil">@lang('users.movil')</label>
                <input type="text" id="movil" name="movil" class="form-control"
                    value="{{ $user->partner->movil }}" maxlength="10"/>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-4">
                <label for="sex">@lang('users.sex')</label>
                <br>
            ...
            <div class="col-lg-4">
                ...
                <input type="text" id="street" name="street" class="form-control"
                    value="{{ $user->partner->street }}" />
            </div>
            <div class="col-lg-2">
                <label for="noExt">@lang('users.exterior_number')</label>
                <input type="text" id="noExt" name="noExt" class="form-control"
                    value="{{ $user->partner->noExt }}" />
            </div>
            <div class="col-lg-2">
                <label for="noInt">@lang('users.interior_number')</label>
                <input type="text" id="noInt" name="noInt" class="form-control"
                    value="{{ $user->partner->noInt }}" />
            </div>
        </div>
        <div class="row">
            <div class="col-lg-4">
                <label for="colony">@lang('users.colony')</label>
                <input type="text" id="colony" name="colony" class="form-control"
                    value="{{ $user->partner->colony }}" />
            </div>
            <div class="col-lg-2">
                <label for="postal_code">@lang('users.postal_code')</label>
                <input type="text" id="postal_code" name="postal_code" class="form-control"
                    value="{{ $user->partner->postal_code }}" />
            </div>
            <div class="col-lg-2">
                ...
                <select id="country" name="country" class="selectpicker" data-live-search="true">
                    @foreach ($countries as $country)
                        ...
                        <option value="{{ $country->id }}"
                            data-subtext="<img src='{{ asset('imgs/flags/' . $flag) }}' width='25px' height='20px' style='margin-left:-92.5%;text-align:left;'>">
                           &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; {{ ucfirst($country->name) }}</option>
                    @endforeach
                </select>
                <input type="hidden" id="country_select" name="country_select"
                    value="{{ $user->partner->country_id }}" />
            </div>
        </div>
        <div class="row">
            <div class="col-lg-3">
...
@section('scripts')
    <script type="text/javascript">
        ...
        //"{{$user->partner->phone_code_number}}"
        document.getElementById('phone_number_code').value = "{{$user->partner->phone_code_number}}";

        $('#phone_number_code').change(function(){
            $('#phone_code_hidden').val($('#phone_number_code').val());
        });
                ...
                if (status = 'success') {
                    if (states == 'no_data') {
                        ...
                        $.each(states, function(position, state) {
                            $('#state').append("<option value='" + state.id + "'>" + state.name.charAt(0)
                                .toUpperCase() + state.name.substring(1) + "</option>");
                            $('#state').selectpicker('refresh');
                        });
                ...
                if (status = 'success') {
                    if (cities == 'no_data') {
                        ...
                        $.each(cities, function(position, city) {
                            $('#city').append("<option value='" + city.id + "'>" + city.name.charAt(0)
                                .toUpperCase() + city.name.substring(1) + "</option>");
                            $('#city').selectpicker('refresh');
                        });
